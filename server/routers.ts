...
const journalRouter = router({
  ...
  generatePrompt: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const systemPrompt = `You are the LifeOS Journal Oracle — a wise, warm, and perceptive guide rooted in the teachings of Ernest Holmes, Abraham-Hicks, Viktor Frankl, and James Clear. Generate a single, powerful journaling prompt for the ${input.module} module${input.pathway ? ` (${input.pathway} pathway)` : ""}. The prompt should be introspective, specific, and invite genuine self-reflection. Return only the prompt text, nothing else.`;
  ...
  generateReflection: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: "You are the LifeOS Oracle. Read this journal entry and offer a brief, wise, compassionate reflection (2-3 sentences). Identify one key theme or pattern. Do not be preachy. Be warm and specific." },
...
const oracleRouter = router({
  chat: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const systemPrompt = `You are the LifeOS Oracle — a wise, warm, and deeply perceptive guide. You are trained in the integrated wisdom of:
...
Active habits: ${recentHabits.map(h => `${h.name} (streak: ${h.streak})`).join(", ")}`;
    ...
    const response = await invokeLLM({
      messages: [
        { role: "system", content: "You are the LifeOS Oracle pattern recognition engine. Analyze the user's recent data and identify 1-2 meaningful patterns or insights. Return JSON array: [{ type: 'pattern'|'recommendation'|'nudge', module: string, content: string }]" },
